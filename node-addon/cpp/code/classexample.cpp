...
ClassExample::ClassExample(const Napi::CallbackInfo& info) : Napi::ObjectWrap<ClassExample>(info)  {
  ...
  if(!info[0].IsNumber()){
    ...
    TestClass* parent_actual_class_instance = example_parent->GetInternalInstance();
    this->testClass_ = new TestClass(parent_actual_class_instance->getValue());
    return;
  }

  Napi::Number value = info[0].As<Napi::Number>();
  this->testClass_ = new TestClass(value.DoubleValue());
}
...
Napi::Value ClassExample::GetValue(const Napi::CallbackInfo& info) {
  ...
  double num = this->testClass_->getValue();
...
Napi::Value ClassExample::Add(const Napi::CallbackInfo& info) {
  ...
  double answer = this->testClass_->add(toAdd.DoubleValue());

  return Napi::Number::New(info.Env(), answer);
}

TestClass* ClassExample::GetInternalInstance() {
